...
768/769
*/
import { _decorator, Component, EffectAsset, Enum, gfx, Graphics, IMaterialInfo, Material, Node, resources, UITransform, Vec3, view } from 'cc';
...
export class SSRLoSComponentRender extends Component {
    ...
    _updateLightEffect () {
            ...
            var material = this.render.getSharedMaterial(0) || new Material(); 
            
            var frameSize = view.getFrameSize(); 
            var visibleSize = view.getVisibleSize(); 
            var retinaFactor = view.getDevicePixelRatio(); 
            // var position = this.target.g.convertToWorldSpaceAR(cc.v2(0, 0)); 
            let position = this.target.getComponent(UITransform)?.convertToWorldSpaceAR(new Vec3(0,0,0)) || new Vec3(0,0,0);

...
export class ssrLoSCore {
    ...
    generateScreenRect() {
        let nodePosition = this.getOwner().getPosition();
        let cameraPosition = this._getCameraScreenPosition();
        //     view.getFrameSize(); 
    ...
    clearRayArray() {
        for (let i = 0, l = this._rayArray.length; i < l; i++) {
    ...
    clearHitPointArray() {
        for (let i = 0, l = this._hitPointArray.length; i < l; i++) {
    ...
    setRadius(value: any) {
        ...
        let updateStrategiesFlag = false;
    ...
    _checkScreenPosition() {
        let screenPosition = this._getOwnerScreenPosition();
    ...
    _isNeedUpdate() {
        ...
        if (this._isNeedCulling) {
            let screenRect = this.generateScreenRect();
            let sightRect = new Rect(this.getOwner().getPosition().x - this._radius,
        ...
        let obstacleDirty = this._processObstacleDirtyDetection();
        return needUpdate || obstacleDirty;
    }

    _processObstacleDirtyDetection() {
        let isAnyObstacleDirty = false;
        for (let i = 0, l = this._obstacles.length; i < l; i++) {
            let obstacle = this._obstacles[i];
            let obstacleDirty = false;
            if (obstacle.isDirtyDetectionOn()) {

                if (obstacle._vertexArrayProvider) {
                    let oldVertexArray = obstacle.getVertexArray();
                    let newVertexArray = obstacle._vertexArrayProvider.call(obstacle._node);
                    if (!obstacleDirty) {
                        if (oldVertexArray.length != newVertexArray.length) {
                            ...
                            for (let m = 0; m < oldVertexArray.length; m++) {
                                let isVertexFound = false;
                                for (let n = 0; n < newVertexArray.length; n++) {
                    ...
                    console.log("new vertex array", newVertexArray);
    ...
    addObstacle(node: any, vertexArray: any, isPolygon: any) {
        let obstacle = this.findObstacle(node);
        if (obstacle) {
    ...
    getSightAreaVertCount() {
        let sightVertCount = 0;
        for (let i = 0, l = this._sightAreaArray.length; i < l; i++) {

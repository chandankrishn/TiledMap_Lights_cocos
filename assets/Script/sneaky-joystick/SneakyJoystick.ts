...
export class SneakyJoystick extends Component {
    ...
    update (dt: any) {
            ...
            if (keyboardSimulator && !sys.isNative) { 
                var moveDirection = 0; 
                var rotationDelta = 0; 
                var velocityScale = new Vec2(1, 1); 
                ...
                if (moveDirection != 0) { 
                    var desiredPosition = new Vec2(0, 0); 
                    var verlocityUnit = new Vec2(Math.cos(misc.degreesToRadians(this.target.angle)), Math.sin(misc.degreesToRadians(this.target.angle))); 
                    var velocity = verlocityUnit.multiplyScalar(speed); 
                ...
                var verlocityUnit = new Vec2(Math.cos(misc.degreesToRadians(degrees)), Math.sin(misc.degreesToRadians(degrees))); 
                var fullVelocity = verlocityUnit.multiplyScalar(speed); 
